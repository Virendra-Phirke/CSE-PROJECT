-- Migration: Extend submit_test_result to log per-question answers into test_question_answers
-- Date: 2025-08-14
-- Requires: test_question_answers table & existing submit_test_result function
...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    VALUES (p_test_id, p_student_id, now(), 'in_progress') RETURNING * INTO v_attempt;
  END IF;
  ...
  FOR v_q IN SELECT id, correct_answer, order_index FROM questions WHERE test_id = p_test_id ORDER BY order_index LOOP
    v_total := v_total + 1;
    -- Ensure p_answers index safe
    IF array_length(p_answers,1) IS NOT NULL AND p_answers[v_idx] = v_q.correct_answer THEN
      v_correct := v_correct + 1;
    END IF;
    -- Log per-question answer (may be NULL / out of range)
    INSERT INTO test_question_answers(attempt_id, test_id, question_id, student_id, selected_option, correct_answer, is_correct)
    VALUES (v_attempt.id, p_test_id, v_q.id, p_student_id, CASE WHEN array_length(p_answers,1) IS NOT NULL THEN p_answers[v_idx] ELSE NULL END, v_q.correct_answer, (array_length(p_answers,1) IS NOT NULL AND p_answers[v_idx] = v_q.correct_answer));
    v_idx := v_idx + 1;
  END LOOP;
  ...
  IF FOUND THEN RAISE EXCEPTION 'RESULT_ALREADY_EXISTS'; END IF;
...
COMMENT ON FUNCTION public.submit_test_result(uuid, uuid, int[]) IS 'Scores & records test result + per-question analytics.';
