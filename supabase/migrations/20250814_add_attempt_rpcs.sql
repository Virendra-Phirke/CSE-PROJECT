-- Migration: Add RPC functions for starting attempts and submitting test results with server-side scoring
-- Date: 2025-08-14
-- Requires: test_attempts table, tests, questions, test_results, profiles with UUID FKs

BEGIN;

-- start_test_attempt: idempotently create or fetch an attempt
...
COMMENT ON FUNCTION public.start_test_attempt(uuid, uuid) IS 'Creates or returns existing test attempt for a student.';

-- submit_test_result: validates attempt, scores answers, inserts result, finalizes attempt
...
BEGIN
  ...
  SELECT * INTO v_attempt FROM test_attempts 
    WHERE test_id = p_test_id AND student_id = p_student_id;
  IF NOT FOUND THEN
    -- Create attempt implicitly (could also raise instead)
    INSERT INTO test_attempts(test_id, student_id, started_at, status)
    VALUES (p_test_id, p_student_id, now(), 'in_progress')
    RETURNING * INTO v_attempt;
  END IF;
...
COMMENT ON FUNCTION public.submit_test_result(uuid, uuid, int[]) IS 'Scores and records a test result server-side.';
